...
class PointNet(nn.Module):
    ...
    def forward(self, input):
        n_pts = input.size()[2]
        # First TNet(3x3) / matrix multiplication
...
class PointNetSeg(nn.Module):
    ...
    def forward(self, input):
        # performing the PointNet (to get the input)
        inputs, matrix3x3, matrix64x64 = self.pointnet(input)
        stack = torch.cat(inputs, 1)
        # Perform the MLPs 1088->512->256->128
        x = self.mlp1(stack)
        x = self.mlp2(x)
        x = self.mlp3(x)
        # Classify the points (128->m)
        output = self.conv(x)
